# SFCDC version: 3.1
# 分离QK的Nc，解耦距离度量(dist_metric)与中心计算方式(centroid_mode)
import torch
...
class SFCDC_Simulator:
    def __init__(self, enabled=False, clusbegin_iter=11, start_iter=12, end_iter=46, centers_q=256, centers_k=256, group_size=4, k0=0.25, k1=0.25, k2=0.50, l_f=16, l_h=16, l_w=16, q_quant_type='block', k_quant_type='token', dist_metric='L2', centroid_mode='mean', diff_mode='queue'):
        ...
        self.dist_metric = dist_metric      # 距离度量: 'L1', 'L2', 'Linf'
        self.centroid_mode = centroid_mode  # 中心计算: 'mean', 'median', 'midpoint'
        self.diff_mode = diff_mode          # 差分补偿：'key' for Q_cK_diff^T, 'query' for Q_diffK_c^T
    ...
    def _compute_core(self, qi, ki, vi, si_q, si_k, CQ, CK, device, dtype, inv_sqrt_d, F_dim, H_dim, W_dim, L):
        ...
        # 分数计算: (B, CQ, D) x (B, CK, D)^T -> (B, CQ, CK)
        ...
        # gather 1: 沿 CQ 维度展开 -> (B, L, CK)
        # gather 2: 沿 CK 维度展开 -> (B, L, L)
        ...
        # 1. 基础项: Qc * Kc^T
        # gather expands to (B, L, L)
        res_d = torch.bmm(QC, KC.transpose(1, 2)).gather(1, c_LQ.unsqueeze(-1).expand(-1,-1,self.centers_k)).gather(2, c_LK.unsqueeze(1).expand(-1,L,-1))
    ...
    def _calculate_centroids(self, data, labels, num_centers, device, dtype):
        """支持多种中心计算方式：mean, median, midpoint"""
        # [Modified] 使用 centroid_mode 决定如何计算中心
        B_sl, L, D = data.shape
        ...
        elif self.centroid_mode == 'midpoint':
    ...
    def _kmeans_basic(self, x, k, n_iter=5, initial_centroids=None, device=None):
        ...
        for _ in range(n_iter):
            ...
            # 2. 中心更新 (Update) -> 使用 centroid_mode
