        ...
        if not SA_config.config.get('enabled', False) or sparse_mask_info is None:
            ...
            for head_idx in range(n):
                ...
                if bootstrap_mode:
                    ...
                    blocks_buffer = torch.empty((total_blocks, B_hw, B_hw), device=device, dtype=A.dtype)
                    block_ptr = 0
                    ...
                    diag_blocks_buffer = torch.empty((total_diag_blocks, B_hw, B_hw), device=device, dtype=A.dtype)
                    block_ptr = 0
                    for batch_idx in range(b):
                        for row in range(num_block_sqrt):
                            start = row * B_hw
            ...
            masked_scores = A.masked_fill(final_mask == 0, -float('inf'))
    ...
    def forward(
        ...
        if SA_config.config.get('enabled', False):
            ...
            if (
                ...
                state = {
                    'mask': torch.zeros(expected_mask_shape, device=device, dtype=torch.uint8),
                    'timesteps': torch.zeros(expected_time_shape, device=device, dtype=torch.long),
                }
                self._sparse_histories[sparse_history_key] = state
            elif state['mask'].device != device:
                state['mask'] = state['mask'].to(device=device, non_blocking=True)
                state['timesteps'] = state['timesteps'].to(device=device, non_blocking=True)
                self._sparse_histories[sparse_history_key] = state

            history_state = self._sparse_histories[sparse_history_key]
            updated_buffer = torch.empty_like(history_state['mask'])
            sparse_mask_info = {
                'mask_past': history_state['mask'],
                'timesteps_past': history_state['timesteps'].detach(),
        ...
        if sparse_mask_info is not None:
            history_state = self._sparse_histories[sparse_history_key]
            history_state['mask'].copy_(sparse_mask_info['updated_mask_past'])
            timestep_now_tensor = torch.full(
                (self.num_layers, 1),
                sparse_mask_info['timestep_now'],
                device=history_state['timesteps'].device,
                dtype=history_state['timesteps'].dtype,
            )
            if history_state['timesteps'].size(1) > 1:
                shifted = torch.cat([history_state['timesteps'][:, 1:], timestep_now_tensor], dim=1)
            else:
                shifted = timestep_now_tensor
            history_state['timesteps'].copy_(shifted)
