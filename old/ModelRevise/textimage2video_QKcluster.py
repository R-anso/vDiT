...
from SAM.attn_utils.attn_QK_cluster import Attn_Cluster_Cfg, Attn_Cluster_Manager
...
class WanTI2V:
    ...
    def _build_attn_cluster_cfg(self, sampling_steps: int) -> Attn_Cluster_Cfg:
        ...
        if user_cfg:
            for k, v in user_cfg.items():
                if k in base_cfg:
                    base_cfg[k] = v
        return Attn_Cluster_Cfg(**base_cfg)
    ...
    def t2v(self,
        ...
        with (
            ...
            attn_cluster_cfg = self._build_attn_cluster_cfg(sampling_steps)
            attn_cluster_manager = Attn_Cluster_Manager(attn_cluster_cfg, self.model.num_layers, self.model.num_heads)
            ...
            # [new] 保存聚类加速比结果
            if self.rank == 0 and attn_cluster_manager.cfg.enable:
                attn_cluster_manager.save_and_plot()
    ...
    def i2v(self,
        ...
        with (
            ...
            B_hw = target_shape[2] * target_shape[3]
            attn_save_cfg = self._build_attn_save_cfg(B_hw)
            attn_cluster_cfg = self._build_attn_cluster_cfg()
            attn_cluster_manager = Attn_Cluster_Manager(attn_cluster_cfg, self.model.num_layers, self.model.num_heads)
