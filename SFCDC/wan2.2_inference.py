...
# 1. Add 'src' to path so 'SFCDC' can be imported by 'wan' modules
sys.path.append(os.path.dirname(current_dir))
# 2. Add 'src/Wan2.2' to path so 'wan' can be imported directly
sys.path.append(os.path.join(os.path.dirname(current_dir), 'Wan2.2'))

import wan
from wan.textimage2video_sfcdc import WanTI2V as WanTI2V_SFCDC
...
def _parse_args():
    ...
    # Logic to load prompt and image from folder if prompt_id is provided
    if args.prompt_id is not None:
        prompt_dir = os.path.join('./tests/prompts', str(args.prompt_id))
        if not os.path.isdir(prompt_dir):
            raise FileNotFoundError(f"Prompt directory not found: {prompt_dir}")

        # Load prompt from text file
        prompt_file_path = os.path.join(prompt_dir, args.prompt_filename)
        ...
        for ext in ['png', 'jpg', 'jpeg', 'webp']:
            potential_path = os.path.join(prompt_dir, f'{args.image_filename}.{ext}')
        ...
        if image_path:
            ...
            # Explicitly set to None if not found, to avoid using example images
            args.image = None
...
def generate(args):
    ...
    if "t2v" in args.task:
        ...
        logging.info("Creating WanTI2V pipeline (SFCDC).")
        wan_ti2v = WanTI2V_SFCDC(
            config=cfg,
            checkpoint_dir=args.ckpt_dir,
            device_id=device,
            rank=rank,
